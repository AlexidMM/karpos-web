...
export const load: PageServerLoad = async ({ fetch, cookies, depends }) => {
    ...
    try {
        ...
        const patientUrl = `http://34.51.19.104:3001/patients/by-user/${userId}?_t=${timestamp}`;
        ...
        const appointmentsUrl = `http://34.51.19.104:3001/patients/appointments/details?_t=${timestamp}`;
